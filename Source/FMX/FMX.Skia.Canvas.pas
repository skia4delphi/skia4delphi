...
begin
  ...
  begin
    ...
    FBitmapSurface := TSkSurface.MakeRasterDirect(TSkImageInfo.Create(LBitmap.Width, LBitmap.Height, SkFmxColorType[LBitmap.PixelFormat]), LBitmap.Pixels, NativeUInt(LBitmap.Width) * NativeUInt(PixelFormatBytes[LBitmap.PixelFormat]));
...
begin
  ...
  begin
    ...
    LParentHandle := WindowHandleToPlatform(Parent);
    if LParentHandle{$IF CompilerVersion < 30}.Form{$ENDIF}.Transparency then
      FSurface.ReadPixels(TSkImageInfo.Create({$IF CompilerVersion < 31}Width, Height{$ELSE}LParentHandle.WndClientSize.Width, LParentHandle.WndClientSize.Height{$ENDIF}, TSkColorType.BGRA8888), LParentHandle.BufferBits, NativeUInt({$IF CompilerVersion < 31}Width{$ELSE}LParentHandle.WndClientSize.Width{$ENDIF}) * NativeUInt(SkBytesPerPixel[TSkColorType.BGRA8888]));
    {$ENDIF}
    FSurface := nil;
...
begin
  LImage := TSkImage.MakeFromRaster(
    ...
    ABitmap.Pixels, NativeUInt(ABitmap.Width) * NativeUInt(PixelFormatBytes[ABitmap.PixelFormat]));
  if LImage <> nil then
...
begin
  ...
  begin
    ...
    begin
      ...
      LWidth  := Integer(Round(Width  * Scale));
      LHeight := Integer(Round(Height * Scale));
      FBitmap := CGBitmapContextCreate(nil, LongWord(LWidth), LongWord(LHeight), 8, LongWord(LWidth) * LongWord(SkBytesPerPixel[TSkColorType.RGBA8888]), FColorSpace, kCGImageAlphaPremultipliedLast);
      if FBitmap = nil then
        Exit(nil);
    end;
    FBackBufferSurface := TSkSurface.MakeRasterDirect(TSkImageInfo.Create(Integer(CGBitmapContextGetWidth(FBitmap)), Integer(CGBitmapContextGetHeight(FBitmap)), TSkColorType.RGBA8888), CGBitmapContextGetData(FBitmap), CGBitmapContextGetBytesPerRow(FBitmap));
    {$ELSEIF DEFINED(MSWINDOWS)}
    if FBitmap = 0 then
    begin
      LWidth  := Integer(Round(Width  * Scale));
      ...
      LBitmapInfo.bmiHeader.biSizeImage   := UInt32(LWidth) * UInt32(LHeight) * UInt32(SkBytesPerPixel[TSkColorType.BGRA8888]);
